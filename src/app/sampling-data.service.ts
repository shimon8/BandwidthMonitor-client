...
export class SamplingDataService {
  ...
  currentSampling$=new BehaviorSubject<CurrentSampling>({bytes_recv:0,bytes_sent:0,current_bytes:0});

  loadLastMinSampling(): Observable<number[]> {

    return this.http.get<SamplingInfo>(this.baseUrl+"/getSamplingInfo")
    .pipe(
        map(res=>{
  ...
  loadLastSampling(): Observable<CurrentSampling> {
    return this.http.get<CurrentSampling>(this.baseUrl+"/getLastSampling").pipe(
      map(res=>res)
      );
  }
  getHighValue(){
